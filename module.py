...
class CAWN(torch.nn.Module):
  def __init__(self, n_feat, e_feat, agg='tree',
         ...
         verbosity=1, get_checkpoint_path=None, walk_linear_out=False, neighbors_builder=None):
    ...
    self.neighbors_builder = neighbors_builder
  ...
  def grab_subgraph(self, src_idx_l, cut_time_l, e_idx_l=None, tgt_idx_l=None):
    subgraph = self.neighbors_builder.find_k_hop(self.num_layers, src_idx_l, cut_time_l, num_neighbors=self.num_neighbors, e_idx_l=e_idx_l, tgt_idx_l=tgt_idx_l)
    return subgraph
  ...
  def forward_msg_layer(self, hidden_embeddings, time_features, edge_features, position_features, masks, attn_m):
    ...
    for i in range(len(edge_features)):
      ...
      new_src_embeddings.append(new_src_embedding[0])
  ...
  def retrieve_position_features(self, src_idx_l, node_records, cut_time_l, t_records, test=False):
    ...
    if self.agg == 'tree':
      ...
      position_feature, common_nodes, _ = encode(np.expand_dims(src_idx_l, 1), np.expand_dims(cut_time_l, 1))
      position_feature = position_feature.unsqueeze(1)
      position_features = [position_feature]
      for i in range(len(node_records)):
        batch, i_hop_neighbors = node_records[i].shape
        position_feature, common_nodes, _ = encode(node_records[i], t_records[i])
        position_feature = position_feature.view(batch, i_hop_neighbors, self.pos_dim)
...
class RandomWalkAttention(nn.Module):
  ...
  def forward_one_node(self, hidden_embeddings, time_features, edge_features, position_features, masks=None):
    ...
    if self.walk_pool == 'sum':
      ...
      # YL attention input
      # X = self.self_attention(X.transpose(1,0))
      # X = X.transpose(1,0)
