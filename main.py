...
for run in range(4):
  ...
  cawn = CAWN2(n_feat, e_feat, memory_dim, max_idx + 1, time_dim=TIME_DIM, pos_dim=POS_DIM, n_head=ATTN_NUM_HEADS, num_neighbors=num_neighbors, dropout=DROP_OUT,
    ...
    history_agg_method=HISTORY_AGG_METHOD, agg_time_delta=AGG_TIME_DELTA, backprop_n_history=BACKPROP_N_HISTORY, n_layers=NUM_LAYER, preprocess=preprocess)
  cawn.to(device)
  ...
  if not preprocess:
    max_e_idx = (max_idx + 1) * (num_neighbors[0] + 1)
  ...
  total_end = time.time()
  print("MAT experiment statistics:")
  if args.mode == "t":
